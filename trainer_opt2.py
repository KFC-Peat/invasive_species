...
def trainer(image_array, label_array):
    ...
    for j in range(bs_t):
        image_batch_t[j,:,:,:] = sp.misc.imresize(image_array[IMG_NUM+j,c1:c2,r1:r2,:],[IMG_SIZE,IMG_SIZE,3])
    ...
    for i in range(epochs):
        ...
        for j in range(bs):
            ...
            # get random dimentions
            c1 = int(random.random()*col/8)
            c2 = c1 + col*7//8
            r1 = int(random.random()*row/8)
            r2 = r1 + row*7//8

            image_batch[j,:,:,:] = sp.misc.imresize(image_array[bs*cb+j,c1:c2,r1:r2,:],[IMG_SIZE,IMG_SIZE,3])
